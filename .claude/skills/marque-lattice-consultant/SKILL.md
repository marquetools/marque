...
| `sources/` | sources index + vendored PDFs | — | `SOURCES.md` lists every primary source (vendored or cite-and-link), license, and origin URL. Two PDFs vendored under explicit redistribution permission (`burris-sankappanavar`, `moller-schwartzbach`); the rest are cite-and-link only — retrieve from the URL when needed. Don't read PDFs during operation; reference via citation key. |
