...
- **File**: `crates/marque-engine/src/engine.rs:217`
...
- **File**: `crates/marque-capco/tests/corrections_map.rs`
...
- **File**: `marque/Cargo.toml:18`, `crates/marque-server/Cargo.toml:18`
- **Description**: `marque-capco = { path = "../crates/marque-capco" }` uses a path dep instead of `workspace = true`. Inconsistent with other workspace deps. Not a bug — both resolve to the same crate.
...
| `crates/marque-capco/tests/corrections_map.rs` | Modified — Phase 5 tests (from prior merge base) |
| `crates/marque-engine/Cargo.toml` | Modified — batch feature flag, aho-corasick dep |
| `crates/marque-engine/src/engine.rs` | Modified — AhoCorasick pre-scanner refactor |
| `crates/marque-engine/src/lib.rs` | Modified — cfg(feature = "batch") gate |
| `crates/marque-server/Cargo.toml` | Modified — features = ["batch"] |
| `crates/marque-wasm/Cargo.toml` | Modified — cleaned deps, console_error_panic_hook |
| `crates/marque-wasm/examples/harness.html` | Added — interactive WASM harness |
| `crates/marque-wasm/src/lib.rs` | Modified — rewritten for NDJSON parity |
| `crates/marque-wasm/tests/native_parity.rs` | Added — SC-008 parity tests |
| `crates/marque-wasm/tests/no_io.rs` | Added — FR-013 dep audit |
