...
29 rules currently implemented: errors `E001`–`E025`, warnings `W001`–`W003`, corrections `C001`. ID prefix encodes default severity (`E` = error, `W` = warning, `C` = correction). Use `CapcoRuleSet::new()` or the `capco_rules()` entry point to obtain the full set.
