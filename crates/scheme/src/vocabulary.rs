...
/// IC-specific vocabulary extension over [`Vocabulary`].
///
/// The base [`Vocabulary`] trait is domain-neutral — it carries no
/// vocabulary specific to any one marking system. Foreign-disclosure-and-
/// release (FD&R) membership is an IC/CAPCO concept (NOFORN / RELIDO /
/// REL TO / DISPLAY ONLY / EYES), not a property every marking scheme
/// has, so it lives here on a sub-trait that an IC scheme opts into
/// rather than on the base trait every scheme must carry. A CUI-only,
/// NATO, or partner-national scheme implements [`Vocabulary`] without
/// implementing [`IcMarkingVocabulary`].
///
/// A scheme implements this in addition to [`Vocabulary`]; the
/// supertrait bound makes a `&dyn IcMarkingVocabulary<S>` usable
/// everywhere a `&dyn Vocabulary<S>` is. `Send + Sync` is inherited from
/// the [`Vocabulary`] supertrait so the engine can hold an
/// `Arc<dyn IcMarkingVocabulary<S>>` across threads.
pub trait IcMarkingVocabulary<S: MarkingScheme + ?Sized>: Vocabulary<S> {
    ...
    /// # No default impl
    ///
    /// Unlike the previous trait-default `false`, this method is
    /// **required**. Moving FD&R onto a dedicated sub-trait means the
    /// only schemes that implement [`IcMarkingVocabulary`] are schemes
    /// that *have* an FD&R family, so the "forgot to override → silent
    /// `false`" footgun is gone: a scheme either declares FD&R
    /// membership (implements this trait and the method) or it does not
    /// (does not implement the trait at all). An impl MUST cover every
    /// token that participates in the FD&R chain AND every category
    /// whose tokens collectively participate (e.g., CAPCO's REL TO
