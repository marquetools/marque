...
//! schema constant.
//!
//! Post PR 3c.2.D (atomic cutover) the accept-list contracts to a
//! single value: `["marque-1.0"]`. The legacy `mvp-1` / `mvp-2` /
//! `mvp-3` shapes retired alongside the v2 `AppliedFix` reshape,
//! BLAKE3 digesting, closed `MessageTemplate` JSON serialization,
//! and `Canonical<S>` provenance wiring, to close the G13 audit-
...
//!      the `AUDIT_SCHEMA_IS_V1_0` discriminant is `true`.
...
#[test]
fn audit_schema_version_is_v1_0_by_default() {
    assert_eq!(marque_engine::AUDIT_SCHEMA_VERSION, "marque-1.0");
}
...
fn audit_schema_is_v1_0_const_matches_version() {
    assert!(marque_engine::AUDIT_SCHEMA_IS_V1_0);
}
...
fn build_rs_accept_list_pinned() {
    ...
    assert!(
        build_rs.contains(r#"const ACCEPTED: &[&str] = &["marque-1.0"];"#),
        "accept-list drifted from `[\"marque-1.0\"]`; \
         coordinate with audit-emit paths before editing build.rs",
    );
    assert!(
        build_rs.contains(r#"const DEFAULT: &str = "marque-1.0";"#),
        "DEFAULT drifted from `\"marque-1.0\"`; coordinate with \
