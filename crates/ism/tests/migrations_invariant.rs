...
//! `MigrationEntry::valid_until` parse-shape invariant test.
//!
...
//! in marque yet. When/if such a type lands (likely as part of a
//! future "evaluate at schema version" rule context flag), this
//! local helper retires in favor of the typed parser.
//!
//! ## Scope note
//!
//! `MigrationEntry` itself carries `valid_until` only; the matching
//! `valid_from` lives on `Deprecation<Token>`
...
// migration struct in `crates/ism/build.rs`. The `MIGRATIONS` static and
...
fn migration_entries_valid_until_parses_as_schema_version() {
    // `MigrationEntry` carries `valid_until`; the
