...
//!   (`§<Letter>.<subsection>[.<sub_subsection>] [Table <table>] p<page>`
//!   — see `tools/citation-lint/src/scanner.rs`), so PR 3c.2.C's
//!   `&'static str → Citation` migration is purely mechanical.
//!
...
/// (`§<Letter>.<subsection>[.<sub_subsection>] [Table <table>] p<page>`).
/// `document` is NOT rendered today because CAPCO-2016 is the only
...
impl fmt::Display for Citation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(sub_sub) = self.section.sub_subsection {
            write!(f, ".{}", sub_sub.get())?;
        }
...
///
/// Accommodates the three CAPCO citation shapes verified against
...
/// - `§<L>.<sub>.<sub_sub>` (e.g., `§H.5.4 p99` — SAR-internal subsection;
///   CAPCO-2016 §H.5 p99 verified at PR 3c.2.A authorship as the SAR
///   anchor — the `.4` sub-subsection shape is reserved for future
///   sub-subsection citations as the rule catalog deepens)
///
/// The `Option<NonZeroU8>` choice for `subsection` / `sub_subsection` /
/// `table` niche-saves the `Option<u8>` tail and statically rejects
/// sentinel-zero. Bare `§H` (no subsection) is representable as
/// `subsection = None`.
///
/// Construction is builder-style: start with [`SectionRef::new`], then
/// chain `with_subsection` / `with_sub_subsection` / `with_table` to
...
pub struct SectionRef {
    ...
    /// Sub-subsection number — e.g., `§H.5.4` → `sub_subsection =
    /// Some(4)`. Always paired with a populated [`Self::subsection`]
    /// (`§H..4` is not a valid citation shape).
...
impl SectionRef {
    ...
    /// `with_subsection` / `with_sub_subsection` / `with_table` to
    ...
    /// Add a sub-subsection number (e.g., `§H.5` → `§H.5.4` via
    /// `.with_sub_subsection(NonZeroU8::new(4).unwrap())`).
    pub const fn with_sub_subsection(self, sub_subsection: NonZeroU8) -> Self {
        Self {
            sub_subsection: Some(sub_subsection),
            ..self
        }
    }
...
mod tests {
    ...
    const _SAR_SUB_SUB: Citation = Citation::new(
        AuthoritativeSource::Capco2016,
