...
impl CapcoScheme {
    ...
    fn build_page_rewrites() -> Vec<PageRewrite<CapcoScheme>> {
        ...
        // §H.8 p134 (FOUO-in-classified clause).
        const PATTERN_B_CLASS_FOUO_READS: &[marque_scheme::CategoryId] =
            &[CAT_CLASSIFICATION, CAT_DISSEM];
        const PATTERN_B_CLASS_FOUO_WRITES: &[marque_scheme::CategoryId] = &[CAT_DISSEM];

        // non-fdr-control-evicts-fouo: reads DISSEM (FOUO + other
        // non-FD&R IC dissem), NON_IC, AEA, SAR; writes DISSEM
        // (FactRemove FOUO). §H.8 p134 (FOUO-with-other-non-FD&R clause).
        const PATTERN_B_NON_FDR_READS: &[marque_scheme::CategoryId] =
            &[CAT_DISSEM, CAT_NON_IC_DISSEM, CAT_AEA, CAT_SAR];
