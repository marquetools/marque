...
///
/// The crate-private `apply_overlays` API takes
/// `DISSEM_SUPERSESSION_TABLE` directly; the only call site is inside
/// `marque-capco`. The `&'static` reference passed everywhere in this
/// module is the load-bearing invariant — no ad-hoc copies of the table.
///
...
///   or DISPLAY ONLY").
/// - §H.8 p157 (EYES ONLY: NSA-only marking). The engine migrates
///   EYES ONLY → REL TO at fix-time, but the parser preserves
///   `DissemControl::Eyes` during lint runs, so EYES survives parse and
///   appears in `dissem_us` during intermediate lattice composition (see
///   `join_via_lattice` in `crates/capco/src/scheme/marking.rs`). NOFORN
///   must therefore dominate EYES ONLY in the supersession table for the
///   lattice path to be correct per §H.8 p145.
static DISSEM_SUPERSESSION_TABLE: &[(DissemControl, DissemControl)] = &[
    ...
    // or DISPLAY ONLY").
    //
    // EYES ONLY is included: the parser preserves `DissemControl::Eyes`
    // (parser lives in `marque-core`); the EYES → REL TO migration is an
    // engine-time rule. During lint runs and intermediate lattice
...
///    rendezvous in `CapcoMarking::join_via_lattice`.
///
/// **Overlay placement** (matching parity-gate fixtures in
/// `crates/capco/tests/lattice_vs_scheme_parity.rs`). The parity gate
/// compares the per-axis lattice path (`project_via_lattice`) against
/// the full scheme pipeline (`project_via_scheme = scheme.project(
/// Scope::Page, ...)`, which runs the declarative PageRewrite catalog
/// over the per-axis composition). The four overlays below land on
/// these two paths:
///
...
///   the lattice path via `DissemSet::with_noforn_injected`.
///   `NonIcDissemSet::from_attrs_iter`'s `needs_nf` flag
...
/// concern, not the lattice's, handled by `MarkingScheme::render_canonical`.
///
...
/// for bottom, leave `top()` undefined." `FgiSet` is NOT part of this
/// precedent — it does not implement `BoundedLattice` at all.
///
/// **Partial-lattice note.** The
...
///
/// **`Default`.** `Default` MUST agree with
...
impl Default for DissemSet {
    /// `Default` MUST agree with `DissemSet::empty()`. See the struct
    /// doc comment for the rationale — a derived `Default` would set
    /// `relido_observed_unanimous = false` (bool's Default) and break
    /// the `from_attrs_iter(&[]) == empty()` agreement.
    fn default() -> Self {
        Self::empty()
    }
}
...
impl DissemSet {
    ...
    /// The `table` parameter MUST be `DISSEM_SUPERSESSION_TABLE`. The
    /// single-static-table convention is enforced by `apply_overlays`
    /// being crate-private with `DISSEM_SUPERSESSION_TABLE` as the only
    /// in-tree caller.
    ...
    /// overlay. Callers that need to inject NOFORN from a cross-axis
    /// source (non-IC SBU-NF / LES-NF on a classified page, NODIS /
    /// EXDIS supersession, or the `capco/noforn-clears-rel-to`
    /// PageRewrite) MUST route through here so the §H.8 p145
    /// NOFORN-dominates rule strips `Rel` / `Relido` / `Displayonly`
    /// from the set. Injecting `Nf` directly into the bag without
    /// re-running the overlay would leave dominated controls in place,
    /// invalid per §H.8 p145.
    ///
    ...
    /// # Why all three
    ///
    /// This is the "rebuild overlays on a post-close /
    /// post-default-fill state" method, not specifically the FD&R
    /// strip — the body runs all three overlays via `apply_overlays`.
    ///
    /// Architectural context (issue #704): the `CapcoScheme::closure`
    /// operator is purely additive (Kleene fixpoint over
    /// `CLOSURE_TABLE`), so it must preserve monotonicity
    /// (`a ⊑ b ⟹ Cl(a) ⊑ Cl(b)`). The "default if absent" semantic
    /// lives in `crate::scheme::default_fill::apply_default_fill`;
    /// the "post-close re-evaluate per-axis supersession" half lives
    /// here, called from `CapcoScheme::apply_supersession_overlays`.
    ///
    /// # Properties
    ///
...
// Partial-lattice divergence note for `DissemSet`.
//
...
// over the full `(set, relido_observed_unanimous)` pair. The `Lattice`
// trait split (issue #456 / PR #502) into `JoinSemilattice` and
// `MeetSemilattice` halves lets `DissemSet` implement only the join half,
// so the type system rejects any attempt to call `.meet()` on it at
// compile time.
//
// See the `DissemSet` doc comment above ("Partial-lattice note") for the
// full rationale.
...
mod with_all_overlays_reapplied_tests {
    ...
    // ---------------------------------------------------------------
    // Overlay 1 (OC > OC-USGOV) must re-run without requiring NOFORN.
    // Gating the dissem-axis rebuild on `has_noforn` would silently
    // disable Overlay 1 for post-close states carrying both `Oc` and
    // `OcUsgov` with no NOFORN — so any future close()/default_fill
    // catalog row that emits `Oc` onto an `OcUsgov`-bearing input (or
    // vice versa) would NOT see Overlay 1 fire. Authority: §H.8 p140
    // ("ORCON takes precedence within the banner line" + "If a portion
    // contains both ORCON and ORCON-USGOV information, ORCON takes
    // precedence in the portion mark").
    // ---------------------------------------------------------------
    ...
    /// A `has_noforn` short-circuit on the production gate would mask
    /// the Overlay 1 strip if `apply_overlays` hadn't already run
    /// during join; this test pins the contract that BOTH overlays fire
    /// on the same call.
    #[test]
