...
pub(crate) fn build_page_rewrites() -> Vec<PageRewrite<CapcoScheme>> {
    ...
    // #559 close-out (PM 2026-05-19): DISPLAY ONLY / ORCON / ORCON-USGOV
    // → RELIDO eviction rows. Placed after `noforn_clears` because
    // the `capco/noforn-clears-fdr-family` row strips RELIDO when
    // NOFORN is present — running it first means our rows are no-ops
    // on NOFORN-bearing pages and only fire on the non-NOFORN-but-
