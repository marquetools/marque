...
`CapcoMarking::join_via_lattice` composes the lattice types component-wise; the production `Lattice::join` still delegates to PageContext until PR 4b-D flips the hot path. The parity gate at `crates/capco/tests/page_context_lattice_parity.rs` covers 69 `#[test]` fixtures (47 byte-identity + 4 documented divergences after PR 4b-C closed G-1 + G-2; 16 PR 4b-C Pattern-B/C declarative-row fixtures driving the new `project_via_scheme` helper — see `CAPCO-CONTEXT.md` §3 for the enumerated divergence list); corpus-fixture parity is deferred to PR 4b-D when `CapcoScheme::project(Scope::Page, ...)` flips to the lattice path.
