...
47 rules currently implemented: errors `E001`–`E060` (core CAPCO, SAR, SCI, NODIS/EXDIS, REL TO list-grammar invariants, RELIDO incompatibility pairs, class-floor catalog walker, SCI per-system catalog walker, non-canonical input walker; `E017`/`E018`/`E019` retired in T035b; `E022`/`E025`/`E027` retired in PR 3b.D into the `E058` class-floor catalog walker; `E042`–`E051` retired in PR 3b.E into the `E059` SCI per-system catalog walker; `E020`/`E023`/`E028`/`E033` retired in PR 3b.F into the `E060` non-canonical input walker; `E040` emitted by the banner-roll-up walker rather than a registered rule ID), style `S001`–`S006` (`S004` is the first suggest-don't-fix-channel rule per issue #235 / #186 PR-3; `S005`/`S006` cover REL TO membership-uncertain reduction per issue #206), warnings `W002`–`W003` (`W001` retired in T035c-14), corrections `C001`. ID prefix encodes default severity (`E` = error, `W` = warning, `S` = style/info/suggest, `C` = correction). The 47 count matches the registered-rule pin in `crates/capco/tests/corpus_parity.rs::rule_count_reflects_registration_changes` (PR 3b.F landed at 50 − 4 retired + 1 walker added = 47). Use `CapcoRuleSet::new()` or the `capco_rules()` entry point to obtain the full set.
