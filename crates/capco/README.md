...
28 rules currently registered at HEAD (post-PR #578 + issues #261/#250/#251 + T044). The exact-set is gated by `crates/capco/tests/post_3b_registration_pin.rs` against the post-T044 wire-string canonical forms. Coverage shape (legacy IDs in parentheses for archaeology — see `legacy-rule-id-map.md` for current wire strings):
