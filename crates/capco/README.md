...
> **Note on rule IDs**: post-T044 (2026-05-22), rule IDs are 2-tuples `(scheme, predicate_id)` of the form `RuleId::new("capco", "<surface>.<category>.<predicate>")`. The canonical wire string `<scheme>:<predicate_id>` (e.g., `"capco:portion.dissem.noforn-conflicts-rel-to"`) is what users see in `.marque.toml` keys, audit-log text output, and grep targets. The historical sections below referencing `E058` / `E059` / `E060` / etc. are **archaeological** — describing the pre-T044 PR history. See [`docs/refactor-006/legacy-rule-id-map.md`](../../docs/refactor-006/legacy-rule-id-map.md) for the 114-row rename table mapping each retired flat-string ID to its 2-tuple successor.
...
30 rules currently registered at HEAD (post-PR #578 + issues #261/#250/#251/#501/#545 + T044). The exact-set is gated by `crates/capco/tests/post_3b_registration_pin.rs` against the post-T044 wire-string canonical forms. Coverage shape (legacy IDs in parentheses for archaeology — see `legacy-rule-id-map.md` for current wire strings):
...
- **SCI per-system catalog rows** (5 active): retired the per-system rules `E042`–`E051` into the declarative walker formerly known as `E059`; rows fire via the engine constraint-bridge.
- **Class-floor catalog rows** (27 active): retired the per-marking class-floor rules `E022`/`E025`/`E027` etc. into the declarative walker formerly known as `E058`; rows fire via the engine constraint-bridge.
...
- **Style/suggest**: `S003`–`S005`/`S007`–`S010` (`S004` is the first suggest-don't-fix channel per issue #235 / #186 PR-3; `S005` covers REL TO membership-uncertain reduction per issue #206 — PR #488 retired its historical S006 companion and migrated S005 to `Phase::PageFinalization`; `S007` covers bare NATO classification per PR 9c.2 / FR-048; `S008` from #559-C1; `S009`/`S010` from #250/#251).
- **Warnings**: `W003`/`W004`/`W034` (`W001` retired in T035c-14 per CAPCO §F; `W002` retired in PR #507 closing #470 per §H.7 p123 — the shape `W002` was warning on is now authorized by the manual; `W004` added in PR 4b-B for joint-disunity-collapse per §H.3 p57 + §H.7 p123).
...
`CapcoMarking::join_via_lattice` composes the lattice types component-wise. Post-PR-4b-D.2 the production hot path drives page aggregation through `scheme.project(Scope::Page, ...)` (which composes the lattice path + closure rules + PageRewrite catalog). Post-PR-4b-E the parity gate at `crates/capco/tests/lattice_vs_scheme_parity.rs` compares `project_via_lattice` (per-axis lattice composition) against `project_via_scheme` (full pipeline including the PageRewrite catalog). See `CAPCO-CONTEXT.md` §3 for the enumerated divergence list — the load-bearing class is the §B.3 Table 2 p21 closure-rule asymmetry: scheme runs `CLOSURE_NOFORN_CAVEATED`, per-axis lattice doesn't.

## Declarative PageRewrite catalog (PR 4b-C addenda)

PR 4b-C added 9 declarative `PageRewrite` rows on `CapcoScheme` covering the §H.6 / §H.8 / §H.9 strip-plus-promote semantics, bringing the catalog total from 14 to 23 rows. **Pattern-C** (Commit 3) — 7 rows for classification-driven strip of UNCLASSIFIED-only controls: `capco/limdis-evicted-by-classified` (§H.9 p170), `capco/sbu-evicted-by-classified` (§H.9 p176), the four UCNI rows (`capco/{dod,doe}-ucni-promotes-noforn-when-classified` + `capco/{dod,doe}-ucni-evicted-by-classified` at §H.6 p116 / p118; promote rows declared before strip rows so the promote's predicate sees UCNI before the strip removes it), and `capco/fouo-evicted-by-classified` (§H.8 p134 classified sub-clause). **Pattern-B** (Commit 4) — 2 structural rows per §H.8 p134 verbatim: `capco/classification-evicts-fouo` (classified-document sub-clause) + `capco/non-fdr-control-evicts-fouo` (UNCLASSIFIED with other non-FD&R control sub-clause). "Non-FD&R" follows `Vocabulary::is_fdr_dissem`'s broad semantic (INCLUDES RELIDO) — distinct from `is_fdr_dominator` (which EXCLUDES RELIDO; see `scheme.rs:5018-5039`). The 4 UCNI rows fix the pre-PR-4b-C `expected_aea_markings` bug where classified-context UCNI stripping silently dropped the §H.6 NOFORN-promotion clause; the load-bearing post-fix test is `pattern_c_dod_ucni_classified_strip_promotes_noforn` in the parity gate.

The 9 new rows + 4 new vocab sentinels (`TOK_PROPIN=143` / `TOK_FISA=144` / `TOK_RAWFISA=145` / `TOK_NNPI=146`, closing #407) are scheduler-validated at `Engine::new`. Post-PR-4b-D.2 + PR 4b-E, `scheme.project(Scope::Page, ...)` is the single source of truth for page roll-up; the PageContext `expected_*` accessor surface retired entirely. PR 4b-C Commit 5 deleted the imperative `expected_dissem_us` Step 3 (FOUO Step) + `expected_aea_markings` UCNI strip branches; PR 4b-E retired the rest of the surface alongside the renderer (`render_expected_banner` migrated to `MarkingScheme::render_canonical(Scope::Page)`). Pattern D (caveated-implies-NOFORN, §B.3 Table 2 p21) is wired via `CLOSURE_NOFORN_CAVEATED` on `CapcoScheme::closure()`. PR 9a (issue #307) added five rules: `E061` (bare HCS at CONFIDENTIAL → contact originator, §H.4 p62), `E062` (bare HCS at S/TS → suggest HCS-O / HCS-P / HCS-O-P, §H.4 p62), `E063` (bare RSV requires compartment, §H.4 p70), `E064` (EYES / EYES ONLY → REL TO conversion, §H.8 p157 + p158), and `E065` (`DeprecatedSciLongFormRule` — HUMINT / COMINT / ECI / EL / ENDSEAL / KDK / KLONDIKE canonicalization, §H.4 pp 61, 62, 74, 76, 78, 85). PR 3c.B Commit 7.3 + 7.4 retired `DeclarativeClassFloorRule` (E058) and `DeclarativeSciPerSystemRule` (E059); the 27 + 5 catalog rows still fire via the engine's constraint-catalog bridge (E058 via the `ConstraintViolation` envelope path; E059 via `CapcoScheme::bridge_sci_per_system_diagnostics` so `FixProposal` survives). PR 3c.B Commit 6 (form-bucket migration) retired 13 hand-written form rules (`E001`/`E003`/`E004`/`E009`/`S001`/`S002`/`E011`/`E013`/`E026`/`E029`/`E030`/`E032`/`E052`) plus the E060 non-canonical input walker into `MarkingScheme::render_canonical`. Previously retired: `E017`/`E018`/`E019` in T035b; `E022`/`E025`/`E027` in PR 3b.D into the (now-retired) `E058` class-floor catalog walker; `E042`–`E051` in PR 3b.E into the (now-retired) `E059` SCI per-system catalog walker; `E020`/`E023`/`E028`/`E033` in PR 3b.F into the (now-retired) `E060` walker; `E035`/`E040` emitted as per-row IDs by the `E031` banner-roll-up walker. ID prefix encodes default severity (`E` = error, `W` = warning, `S` = style/info/suggest, `C` = correction). The current registered count (30 post-#578 + #261/#250/#251/#501/#545 + T044) is gated by the count pin in `crates/capco/tests/corpus_parity.rs::rule_count_reflects_registration_changes` and the exact-ID-set pin in `crates/capco/tests/post_3b_registration_pin.rs` (post-T044 the set is keyed by 2-tuple wire strings; the historical "39 rule IDs" pin name in this paragraph reflects pre-T044 history). Use `CapcoRuleSet::new()` or the `capco_rules()` entry point to obtain the full set.
