...
//!   duplicated. Tiebreaker: E002 < E052 → E002 wins.
//! - E020 (`country-code-ordering`) and E052 can fire on the same span
//!   when REL TO is misordered AND a code is duplicated. Tiebreaker:
//!   E020 < E052 → E020 wins.
//!
//! For these overlaps to converge in a single fix pass (the project's
//! idempotency invariant), the surviving fix MUST itself produce
//! canonical output (USA-first + alphabetical + unique). PR-B's first
//! pass left E002 and E020 emitting sorted-but-still-duplicated
//! replacements, which would have been re-flagged by E052 on the next
//! lint — breaking single-pass idempotency.
//!
//! The fix (this PR's commit on top of PR-B): both E002 and E020 now
//! compose `dedup_country_codes` + `canonicalize_trigraph_list` in their
//! fix paths, while their CHECK paths still gate on misorder/missing-USA
//! alone (so E052 retains sole responsibility for duplicate detection
//! diagnostics).
//!
...
// ---------------------------------------------------------------------------
// E020 + E052 overlap — the core single-pass idempotency case
// ---------------------------------------------------------------------------

#[test]
fn e020_and_e052_overlap_resolves_in_single_pass() {
    // Misordered AND duplicated: GBR appears twice and the first GBR is
    // out of alphabetical order. Both E020 and E052 fire on the same
    // REL TO trigraph span. FR-016 lex tiebreaker keeps E020; with the
    // PR-B fixup, E020's replacement is canonical (`USA, AUS, GBR`) so
    ...
    assert_eq!(
        ...
        "single pass must produce the fully canonical REL TO list"
    );
    // Exactly one of {E020, E052} survives the C-1 overlap guard. Under
    // FR-016 it's E020. The other rule's fix is dropped, but the rule
    // assertion below stays open to either survivor — the contract is
    // "the surviving fix is canonical", not "E020 specifically wins".
    let rel_to_fixes: Vec<&String> = applied
        .iter()
        .filter(|r| matches!(r.as_str(), "E020" | "E052"))
        .collect();
    assert_eq!(
        rel_to_fixes.len(),
        1,
        "C-1 overlap guard must keep exactly one of E020/E052: {applied:?}"
    );
}

#[test]
fn e020_and_e052_overlap_is_idempotent_under_re_fix() {
    // Idempotency invariant: applying `fix` twice produces the same
    // output as applying it once. Re-running `fix` on the canonical
    // output must produce no E020 or E052 fixes.
    let source = "SECRET//REL TO USA, GBR, AUS, GBR\n";
    ...
    assert_eq!(
        ...
        "fix must be a fixed-point: a second fix pass produces no further changes"
    );
    let rel_to_fixes_in_second: Vec<&String> = applied_second
        .iter()
        .filter(|r| matches!(r.as_str(), "E002" | "E020" | "E052"))
        .collect();
    assert!(
...
fn e052_alone_produces_canonical_output_when_input_is_in_order() {
    ...
    // appears twice. E020 does NOT fire (sort-only check passes
    // because USA, AUS, AUS, GBR sort-stable equals itself). Only E052
    // fires; its dedup-only replacement is canonical because the input
    // was already canonical modulo the duplicate.
    ...
    assert!(
        !applied.iter().any(|r| r == "E020"),
        "E020 must NOT fire on an in-order list: {applied:?}"
    );
}

// ---------------------------------------------------------------------------
// E020 alone — misordered without duplicates
// ---------------------------------------------------------------------------

#[test]
fn e020_alone_still_canonicalizes_when_input_has_no_duplicates() {
    // Pure misorder, no duplicates. E020 fires alone. The fixup's
    // dedup-then-canonicalize path is a no-op for the dedup step;
    // output is the sorted list. This pins that the dedup compose
    // doesn't accidentally reshape inputs that have no duplicates.
    let source = "SECRET//REL TO USA, GBR, AUS\n";
    let (fixed, applied) = fix_once(source);

    assert_eq!(
        fixed, "SECRET//REL TO USA, AUS, GBR\n",
        "E020 alone must produce sorted output (dedup is a no-op when input has no duplicates)"
    );
    assert!(
        applied.iter().any(|r| r == "E020"),
        "E020 must fire on misordered input: {applied:?}"
    );
    assert!(
        !applied.iter().any(|r| r == "E052"),
...
fn multi_block_dedup_reaches_fixed_point_in_single_pass() {
    // Two REL TO blocks, each independently misordered AND duplicated.
    // E020 detects multiple REL TO blocks (`rel_to_blocks.len() > 1`)
    // and takes the suppression branch — it emits a diagnostic but no
    // fix, because a flat first→last splice across blocks would delete
    // the intervening `//...//` content. E052 is the only fix-emitting
    // rule on this input; it scopes each diagnostic per block, so each
    // block's duplicates are collapsed independently and the
    // intervening `//NF//` (which E001 expands to `//NOFORN//`) is
    // preserved verbatim.
    //
    // The idempotency contract this test pins: the multi-block
    // dedup-only path reaches a fixed point in one fix pass. Per-block
    // ordering is NOT canonicalized here — that would require lifting
    // E020's multi-block suppression, which is out of scope for the
    // PR-B fixup. The block-level orderings stay as the user wrote
    // them, minus duplicates.
    ...
    assert_eq!(
        ...
        "each block must dedup independently; E001 expands NF→NOFORN; \
         E020 suppresses on multi-block input so per-block ordering is preserved"
    );

    // Idempotency: re-running fix produces no further changes. E020
    // still suppresses (still multi-block); E052 has nothing to dedup.
...
// ---------------------------------------------------------------------------
// Audit-record content: applied fix carries E020 or E052 (whichever wins)
...
fn applied_fix_for_overlap_carries_audit_provenance() {
    ...
    let rel_to_applied: Vec<_> = result
        .applied
        .iter()
        .filter(|f| matches!(f.proposal.rule.as_str(), "E020" | "E052"))
        .collect();
    assert_eq!(
        rel_to_applied.len(),
        1,
        "exactly one rule in {{E020, E052}} survives the overlap guard"
    );
    ...
    assert_eq!(
        applied.proposal.replacement.as_ref(),
        "USA, AUS, GBR",
        "the surviving fix's replacement must be canonical (USA-first + alpha + unique)"
    );
}
