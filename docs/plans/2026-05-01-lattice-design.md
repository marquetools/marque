...
10. **NEW (2026-05-07) — §3 (Dissem set): RELIDO incompatibility
    ...
    the §3 fill-in MUST reference the family-predicate framing
    (FD&R-family domination + non-US-family conflict per
    `marque-applied.md` §3.4.2). Q-3.4.2-timing resolved per
    `2026-05-07-pr3b-consultation-verdict.md`: family-predicate
    variant lands here in PR 3.7; PR 3b ships the enumerated form
    (~15–20 single-token rows) and PR 4 compacts to 2 family rows.
11. **NEW (2026-05-07; resolved same day) — §6 (FgiSet): consensus-
    or-fallback already models §4.8.** User confirmation 2026-05-07:
    the FGI category (distinct from "non-U.S. portions for
    non-ICD-206-compliant portions") is the union of acknowledged
    foreign-government trigraphs unless any portion is source-
    concealed, in which case the banner falls back to bare `FGI`.
    This is what existing `FgiSet::Present { concealed, countries }`
    implements (`concealed=true ∨ x = concealed=true` for the
    fallback; `{countries: A} ∨ {countries: B} = {countries: A ∪ B}`
    for the acknowledged-trigraph union). `tasks.md` T108d collapses
    to **doc-comment amendment only** at `crates/capco/src/lattice.rs`
    citing `marque-applied.md` §4.8 + CAPCO §H.7 / §H.7 p123. No new
    primitive; Q-FgiSet-vs-§4.8 closed. Q-5.3 (representable-but-
    unreachable state) folds into the same doc-comment edit.
...
and the new-primitive fold-in. `.claude/skills/marque-lattice-
consultant/references/marque-applied.md` §3.11 is the source of
truth for stage sequencing.
