...
6. **Remediation tier distribution (post-OTHER-1 / #583 flamegraph,
   ...
   candidates (DI-3 RESOLVED as false positive; CO-2 soft-closed;
   CA-1 stays INVESTIGATE as open tech debt; HOT-2 stays INVESTIGATE — the closure operator is
...
- Score is **rough ranking signal**, not an SLA. After the OTHER-1
  ...
  (DI-3 — false positive confirmed), soft-close (CO-2 —
...
1. ~~**Real-flamegraph hot-path attribution** (current: synthesized).
   ...
   **→ OTHER-1**.~~ **RESOLVED 2026-05-19 via PR #583** (release-
   ...
   DI-3 RESOLVED as false positive; CO-2 soft-closed; CA-1 stays
   INVESTIGATE as design-deferred tech debt; HOT-2 stays INVESTIGATE (closure inlined into lattice frames
