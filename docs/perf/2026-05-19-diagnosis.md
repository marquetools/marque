...
6. **Remediation tier distribution (post-OTHER-1 / #583 flamegraph,
   ...
   candidates (DI-3 RESOLVED as false positive; CO-2 + CA-1
   soft-closed; HOT-2 stays INVESTIGATE — the closure operator is
...
`Engine::with_clock` does NOT appear in any top-15 (DI-3 is a false
positive, RESOLVED below); `Engine::two_pass_fix` is 0% on lint
(off-path, as predicted); `Parser::parse_marking_string` is correctly
...
- Score is **rough ranking signal**, not an SLA. After the OTHER-1
  ...
  stays INVESTIGATE. The remaining candidates either RESOLVE
  (DI-3 — false positive confirmed) or soft-close (CO-2 + CA-1 —
  no surface ≥ 4.25% cutoff) or stay INVESTIGATE with tighter
...
1. ~~**Real-flamegraph hot-path attribution** (current: synthesized).
   ...
   **→ OTHER-1**.~~ **RESOLVED 2026-05-19 via PR #583** (release-
   ...
   DI-3 RESOLVED as false positive; CO-2 + CA-1 soft-closed;
   HOT-2 stays INVESTIGATE (closure inlined into lattice frames
   on lint, no discrete surface).
