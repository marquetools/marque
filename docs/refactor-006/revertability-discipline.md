...
| 1 | N | Single-pass forward splice in `Engine::fix_inner` (landed pre-006 at PR #277 / #278); verification at PR 1 of the refactor sequence | Splice-correctness corpus regression coverage at `crates/engine/tests/two_pass_invariants.rs` + `crates/engine/tests/fix_two_pass.rs` | No revert applicable — PR 1 of 006 carried no production code change; it was the verification step that the pre-existing splice (PR #277 / #278) survived the rest of the refactor sequence. Reverting the PR 1 commit is a no-op against the splice itself. If a future maintainer needs to back out the underlying splice, revert PR #277 + #278 on `main` and re-run the corpus regression matrix. |
