...
- **Property-based testing** (`proptest`): 33 tests across three
  integration suites, exercising algebraic laws and engine invariants
  over generated inputs rather than fixed samples.
  - *Lattice laws* (`crates/capco/tests/proptest_lattice.rs`): join
    and meet laws (idempotent, commutative, associative, identity) for
    `SciSet`, `SarSet`, and `FgiSet` over generated compartment trees.
    `FgiSet` additionally covers `BoundedLattice` laws (bottom absorbs
    meet, top propagates join) and concealment monotonicity.
  - *Engine invariants* (`crates/engine/tests/proptest_engine.rs`):
    ...
    Engine is constructed once via `OnceLock` to avoid re-building the
    Aho-Corasick automaton per test case.
  - *PageContext roll-up* (`crates/ism/tests/proptest_page_context.rs`):
    classification monotonicity, dissem-control union superset, and
    REL TO intersection property over generated `IsmAttributes` vectors.
...
| 0.15 | 2026-04-26 | Property-based testing layer added (`proptest` v1). Three new integration suites (33 tests): `crates/capco/tests/proptest_lattice.rs` (21 tests — lattice laws for `SciSet`/`SarSet`/`FgiSet` over generated compartment trees, including `BoundedLattice` laws and concealment monotonicity for `FgiSet`); `crates/engine/tests/proptest_engine.rs` (8 tests — never-panic, span bounds, fix idempotency, dry-run/apply parity, dry-run source-unchanged, threshold enforcement, confidence bounds); `crates/ism/tests/proptest_page_context.rs` (4 tests — classification monotonicity, dissem-control union superset, REL TO intersection property). `proptest = { version = "1", default-features = false, features = ["std"] }` added to workspace dependencies; per-crate dev-dependency added for `marque-capco`, `marque-engine`, `marque-ism`. §14 updated. | Adam Poulemanos (with Claude Code) |
