...
**Document version**: 0.13 · **Last amended**: 2026-04-25
...
3. Pre-fix-loop and per-fix-application, in `Engine::fix_inner`.
   The lint pass completes in full before fix application begins,
   so the `partial_lint` carried in `EngineError::DeadlineExceeded`
   is **always** a complete `LintResult` — only the fix application
   is partial.
...
|---|---|---|
| CLI (`marque`) | `--deadline <humantime>` parsed at `clap`; `Instant::now() + d` per invocation | `EX_TEMPFAIL` (75) on fix expiry; stderr warning + truncated render on lint expiry |
