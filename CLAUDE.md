...
## Recent Changes
- **Issue #677** (2026-05-22): `PortionFormInBannerRule` (`capco:banner.metadata.uses-portion-form`) + `BannerFormInPortionRule` (`capco:portion.metadata.uses-banner-form`) — restored the form-mismatch detection retired in PR 3c.B Commit 6. Commit 6's premise was structurally correct (the renderer's fix path IS in place), but no rule emitted the `Recanonicalize` `FixIntent` that would trigger it. Result: `SECRET//NF`, `(S//NOFORN)`, `SECRET//OC`, `S//NOFORN`, and parallel cases produced zero diagnostics on the pre-fix tree. Both rules walk `attrs.token_spans` and dispatch through `MARKING_FORMS` helpers (`portion_to_banner` / `banner_to_portion`) whose built-in `f.banner != f.portion` gate is the universal filter; the banner rule additionally reads `attrs.classification` to catch US classification shorthand (S/TS/C/U/R) which lives on `Classification::portion_str` / `banner_str`, not in `MARKING_FORMS`. Severity::Fix, confidence 1.0 (deterministic per §G.1 Table 4 closed set), `Phase::WholeMarking`, emits ONE diagnostic per offending marking with `ReplacementIntent::Recanonicalize { Page | Portion }` — the engine's `render_canonical` re-emits the entire marking covering every wrong-form token in one pass. Banner rule suppresses bare EYES / EYES ONLY (E064 owns the §H.8 p157 + p158 cross-axis FVEY conversion); portion rule does NOT suppress (E064 doesn't fire in portion context per §H.8 p158, so the portion-form fallback is strictly better than silent acceptance). Atomically removes the dead-by-construction `is_abbreviation_expansion` guard at the E006 call site (`MIGRATIONS` holds only `25X1-` / `50X1-` declass-shorthand entries since T035c-4; `is_dissem_replacement` filters both BEFORE the form guard could fire). Renderer correctness fix: `DissemControl::Dsen` banner-form was `"DSEN"`, corrected to `"DEA SENSITIVE"` per §H.8 p159 line 3905 worked banner example (the pre-fix Dsen banner-form made the `Recanonicalize { Page }` fix a no-op on `SECRET//DSEN` inputs). Registration pin updated 30 → 32; `phase_assignment.rs` allowlist updated; corpus parity count updated; `EXPECTED_UNCOVERED` whitelist entry for §D.1 p27 retired (now harvested by the new banner rule's primary citation). Authority: §D.1 p27 line 560 (banner permits Marking Title OR Authorized Abbreviation) + §C.1 p25 line 503 (portion uses Register Portion Mark) + §G.1 Table 4 p38 (Register closed-set authority). PM-4 sister bug (`S//NOFORN` classification abbreviation in banner) **subsumed by broad scope** — the banner rule's classification branch catches it via the `attrs.classification` + token-text match; no separate issue. SI-EU/SI-NK SCI compartment cases revealed a MARKING_FORMS data bug in `marque-ism` where the `banner` field records the Marking Title (`SI-ECRU` / `SI-NONBOOK`) instead of the Authorized Banner Abbreviation (`SI-EU` / `SI-NK`) per §H.4 p78 / p83; broad-scope SCI coverage deferred to the follow-up that closes #701, with non-IC dissem (LIMDIS↔DS) and NATO classification (NATO SECRET↔NS) form-pair tests added in the meantime via the form-mismatch test files. Each citation re-verified against `crates/capco/docs/CAPCO-2016.md` at authorship per Constitution VIII.
