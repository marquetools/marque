...
coordinated `marque-2.0` audit-schema bump per FR-049 for any
change:
...
- **Rule-ID 2-tuple form** — the original PR 3c plan called for
...
MVP complete. Full lint → fix → audit pipeline for raw text with **25 registered CAPCO rules** at HEAD (post-PR #578 + issue #261) — `38 post-PR-4b umbrella − 15 PR #578 declarative-wrapper retirements (E010/E012/E014/E015/E016/E021/E024/E036/E037/E038/E053/E054/E055/E056/E057) + S008 #559-C1 + E071 #261 = 25`; the post_3b_registration_pin gates this exact set. See `crates/capco/README.md` for the authoritative rule inventory. Rule-collapse history: W001 retired in T035c-14 per CAPCO-2016 §F; E052 added in issue #234 PR-B per §H.8; S004 trigraph-suggest added in PR-C of #186 per §H.8 (first consumer of the suggest-don't-fix channel); PR 3b.A collapsed three banner roll-up rules (E031/E035/E040) into the `BannerMatchesProjectedRule` walker (top-level `Rule::id() = E031`, still emitting all three per-row IDs); PR 3b.D collapsed class-floor rules (E022/E025/E027) into the `DeclarativeClassFloorRule` walker (E058); PR 3b.E collapsed the SCI per-system rules (E042–E051) into the `DeclarativeSciPerSystemRule` walker (E059); PR 3b.F collapsed the ordering rules (E020/E023/E028/E033) into the `DeclarativeNonCanonicalInputRule` walker (E060). CLI (`check`, `fix`) and WASM (`lint`, `fix`) produce byte-identical NDJSON diagnostics. Configurable severity overrides, corrections map, and confidence thresholds. Batch processing via `BatchEngine` with concurrency control. Criterion benchmarks validate interactive latency p95 ≤ 16 ms on 10 KB single-portion inputs (SC-008) and `fix_throughput` linear scaling (SC-009). Corpus accuracy harness enforces ≥ 95% lint and fix accuracy per-rule against the invalid-fixtures corpus. `cargo-fuzz` target exercises `Engine::lint` on arbitrary `&[u8]`.
...
**Audit schema**: `MARQUE_AUDIT_SCHEMA` env var pinned at build time, validated against the closed accept-list `["marque-1.0"]`. Defaults to `"marque-1.0"` — the final cutover landed at PR 3c.2.D (atomic step `mvp-3 → 1.0`; preceded by `mvp-2 → mvp-3` at PR 3c.B Commit 10 when `FixProposal` retired and the audit envelope took its structural `proposal: FixIntent | TextCorrection` sub-object form, closing the G13 audit-content-ignorance channel). Re-exported as `marque_engine::AUDIT_SCHEMA_VERSION`. A single binary emits exactly one schema (FR-014).
