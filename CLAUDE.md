...
- `Codec<S>` (`marque-scheme`) — pinned trait surface for grammar serialization (encode/decode round-trip). No concrete impls in-tree yet; XML and JSON are planned. `Codec::decode` returns `Parsed<S::Marking>` so ambiguity preserves through the codec layer.
