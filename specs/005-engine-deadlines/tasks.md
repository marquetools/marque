...
- [x] T017 [P] Test `fix_with_deadline_during_apply_loop_returns_deadline_exceeded_with_partial_lint` in `crates/engine/tests/deadline.rs`: deadline crafted to fall AFTER the lint pass but BEFORE all fixes are applied; assert `Err(DeadlineExceeded)` carrying the full lint diagnostics. Uses `FixMode::Apply` on a banner-heavy fixture so the apply phase's `Vec::splice` cost is large enough to make the per-fix deadline check observably trip.
