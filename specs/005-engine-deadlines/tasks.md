...
- [x] T045 `BatchOptions` is now `#[non_exhaustive]` and gains `per_doc_deadline: Option<Duration>`. `Default` initializes it to `None`. The doctest that previously brace-constructed `BatchOptions` was updated to use `Default::default() + field assignment` to preserve outside-the-crate construction across the `#[non_exhaustive]` boundary.
- [x] T046 `BatchEngine` stores `per_doc_deadline` from construction-time options. `lint_many` / `fix_many` honor it via shared `lint_many_inner` / `fix_many_inner` helpers. New `lint_many_with_options` / `fix_many_with_options` variants accept a per-call `&BatchOptions` to override the construction-time default; other fields are reserved for future per-call concerns.
- [x] T047 `BatchError::DocumentDeadlineExceeded { partial_lint: LintResult }` variant added. `is_deadline_exceeded()` predicate added. `is_panic()` / `is_cancelled()` / `is_shutdown()` updated to return `false` for the new variant. `Display` renders "document deadline exceeded after N/M candidates"; `Error::source` returns `None` (matches `EngineError::DeadlineExceeded` shape).
- [x] T048 Per-doc execution stamps `Instant::now().checked_add(d)` AFTER permit acquisition (so concurrency-controller wait does not consume the budget). `lint_many` threads via `LintOptions { deadline, ..Default::default() }`; `fix_many` threads via `FixOptions`. `Err(EngineError::DeadlineExceeded { partial_lint })` from `fix_with_options` maps to `BatchError::DocumentDeadlineExceeded { partial_lint }`. `EngineError::InvalidThreshold` is `unreachable!()` (BatchEngine sets no `threshold_override`); future `EngineError` variants force a non-exhaustive-match error so an explicit `BatchError` mapping is the only way to land them.
- [x] T049 [P] `batch_per_doc_deadline_isolates_one_slow_doc_from_rest` + `batch_lint_many_truncates_slow_doc_without_erroring` + `batch_no_deadline_runs_all_docs_to_completion` in `crates/engine/tests/batch_deadline.rs` (new file). 3 docs, 50 ms per-doc deadline, 20 000-banner slow doc. Slow doc trips the deadline (`DocumentDeadlineExceeded` for fix, `Ok(LintResult { truncated: true })` for lint); fast docs complete on their own budget. Sanity test confirms `per_doc_deadline: None` (default) doesn't accidentally truncate.
- [x] T050 [P] `document_deadline_exceeded_is_matchable_and_distinguishable` + display + source + negative-cross-check tests in the same file. Pin the variant's match-arm shape, predicate behavior (`is_deadline_exceeded()` true, `is_panic()` / `is_shutdown()` / `is_cancelled()` false), `Display` carrying counts and the word "deadline", and `source()` returning `None`.
