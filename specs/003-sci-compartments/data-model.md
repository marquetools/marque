...
/// The bare SCI control systems derived from the live ODNI CVE
/// (`CVEnumISMSCIControls.xml`) at build time. Generated by
/// `build.rs` alongside `SciControl`; includes every CVE value whose
/// text contains no `-`. The 2022-DEC CVE populates this enum with
/// `Bur, Hcs, Klm, Mvl, Rsv, Si, Tk` (7 variants); the 2016 manual
/// authorized only 4 (HCS, RSV, SI, TK) — the tool follows the live
/// CVE, not the manual, because the CVE is the authoritative
/// recognition vocabulary at runtime. Grammar rules (syntax, sort
/// order, banner roll-up) still follow the 2016 manual regardless
/// of which bare values the CVE currently lists.
...
3. Both `SciControlBare` and `SciControl` are generated from the live CVE at build time; `SciControlBare` is the subset whose values contain no `-`. Neither enum has a fixed cardinality — both grow or shrink as the CVE evolves.
