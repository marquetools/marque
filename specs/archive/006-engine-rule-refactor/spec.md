...
- **FR-023**: `Phase::WholeMarking` rules whose span overlaps a pass-1 fix MUST re-validate against pre-pass-1 attributes (cached from pass-0; supplied via `RuleContext.pre_pass_1_attrs: Option<&CanonicalAttrs<'src>>`) before firing. Disambiguation when the predicate holds against both pre- and post-pass-1 attrs: same `RuleId` (i.e., same `(scheme, predicate_id)` 2-tuple per FR-026) → do not re-fire; different rule → fire (different predicate that pass-1 didn't address).
...
- **FR-026** (landed at T044, 2026-05-22): Rule identifiers use the `(scheme, predicate_id)` 2-tuple form (e.g., `RuleId::new("capco", "banner.classification.usa-trigraph")`). The migration from flat `E###`/`W###`/`S###`/`C###` strings landed atomically in T044 alongside the `marque-1.0 → marque-2.0` audit-schema bump. The full rename table is `docs/refactor-006/legacy-rule-id-map.md`; the predicate-ID convention (`<surface>.<category>.<predicate>` with `<surface>` ∈ `{banner, portion, page, marking, closure}`) and the reserved sentinel schemes (`"engine"`, `"test"`) are pinned in `docs/refactor-006/2026-05-22-T044-rule-id-tuple-plan.md` §1.
- **FR-043** (amended 2026-05-14 in lockstep with FR-035a): `MarkingScheme::canonicalize(parsed: ParsedAttrs<'_>) -> CanonicalAttrs` MUST be the single explicit trait-method path that converts parser output into the canonical form rules consume. PR 3a's `from_parsed_unchecked` adapter exists transitionally during the keystone window (3a → 3c.B Commit 10 → PR 3c.2) and **MUST delete at PR 3c.2** (the deletion milestone moved from PR 3c → PR 3c.2 along with the rest of the `marque-1.0` keystone work per FR-035a; 27 surviving call sites at the 2026-05-14 inventory). Post-keystone (post-PR-3c.2), rule crates MUST consume `CanonicalAttrs` produced only via `MarkingScheme::canonicalize` — no public `ParsedAttrs → CanonicalAttrs` constructor MAY exist outside the trait. Canonicalization is a scheme decision; rule crates do not own it.
- **FR-049** (amended at T044, 2026-05-22): `(scheme, predicate_id)` rule identifiers are **stable within a major audit-schema version**. Per **decision D6** in `decisions.md`, the freeze began at PR 10 merge with the 1-tuple string form on `marque-1.0`; T044 unfroze the surface for a single atomic PR that landed the 2-tuple form and bumped `marque-1.0 → marque-2.0`. The freeze re-engages at T044's merge with `marque-2.0` as the new inflection. Any subsequent rename — additive (`marque-2.1`) or breaking (`marque-3.0`) — requires a coordinated audit-schema bump; `docs/refactor-006/legacy-rule-id-map.md` is a living document that gains a row per rename, never silently rewritten (see plan §5 R-4).
- **FR-052**: `MarkingScheme::render_canonical`'s `RenderContext` parameter MUST carry an explicit `EmissionForm` selector — a closed `#[non_exhaustive]` enum with at minimum `Auto | Portion | BannerTitle | BannerAbbreviation`, named per CAPCO §G.1 Table 4 column terms (Marking Title col 1, Banner Line Abbreviation col 2, Portion Mark col 3). `EmissionForm::Auto` MUST derive the emission form from `Scope` (Page → `BannerAbbreviation` if present else `BannerTitle`; Portion → Portion), preserving pre-3c.2 emission behavior for every existing `FixIntent` site (matches the existing `Vocabulary::banner_form()` "abbreviation when distinct, else title" semantics). The closed-CVE branch of `render_canonical` MUST honor the selector by routing to the matching `FormSet` field (PR 3d). When `EmissionForm::BannerAbbreviation` is requested for a token whose `forms().banner_abbreviation == None` (e.g., classifications, FISA, RELIDO), the branch MUST fall back to `forms().banner_title` rather than panicking; `BannerTitle` and `Portion` are total per token, so no fallback rule is needed for those variants. `#[non_exhaustive]` reserves the budget for future variants (e.g., `IsmDescriptionTitle`, `XmlAttribute`, scheme-specific forms) without an API break — adding a variant MUST NOT require touching `FixIntent` emission sites that pass `EmissionForm::Auto`. Rationale: `render_canonical` is the single open-vocab emission path being introduced in PR 3c.2 (T048); locking the selector shape now is essentially free, retrofitting later means re-signing `render_canonical` and propagating into every rule's `FixIntent` emission.
...
- **FR-050**: At PR 10, all per-PR bench comparisons MUST be re-run against the PR-0 baseline (per `research.md` R-5). Per-bench cumulative drift ≤10% is the end-state gate; per-PR contributions exceeding 6% MUST be flagged for attribution. The bench-runner pin (per **decision D8** in `decisions.md`) is `ubuntu-latest` on GitHub Actions hosted runners — hardware pinning beyond the runner family is out of project budget. The cumulative-drift gate stays at 10% and the project explicitly accepts shared-runner image-rotation variance; if the gate flaps in practice, widen the tolerance in a follow-up amendment with the runner-variance-attributed delta documented separately. The bench-runner owner is recorded in the PR 0 description.
...
- **FR-035** (amended 2026-05-14 after PR 3c.B Commit 10; amended again 2026-05-14 per D-7.22; amended 2026-05-22 per T044): PR 3c.B Commit 10 actually landed a conservative `marque-mvp-2 → marque-mvp-3` bump carrying only the `proposal` sub-object discriminated envelope (`FixIntent | TextCorrection`) and the content-ignorance closure (top-level `original`/`replacement` byte fields retired). The fuller post-keystone `marque-1.0` cutover — `Canonical<S>` provenance wired into audit emit, BLAKE3 audit-record digesting, closed-set `MessageTemplate` JSON serialization, `from_parsed_unchecked` adapter deletion — landed at **PR 3c.2** (see FR-035a). The `(scheme, predicate_id)` 2-tuple `RuleId` form landed at **T044** (2026-05-22) alongside the coordinated `marque-1.0 → marque-2.0` audit-schema bump; the FR-049 stability freeze re-engages at T044's merge with `marque-2.0` as the new inflection (see FR-026 and `docs/refactor-006/legacy-rule-id-map.md` for the rename table). `marque-mvp-3` reserved a slot for the R002 diagnostic class that PR 7b filled. The originally-planned `FeatureId::PrecedingFixPenalty` reservation was retired per D-7.22 (misunderstanding-derived; the originally-planned audit-trace path was dead code under current `Phase::Localized` rules and the engine consumer was removed in PR 7c).
- **FR-035a** (added 2026-05-14; amended 2026-05-22 per T044): PR 3c.2 bumped `marque-mvp-3 → marque-1.0` and baked in the four structural commitments deferred from PR 3c.B Commit 10: (a) `Canonical<S>` provenance wired into audit emit paths (`AppliedFix` carries the `discriminant: "strict" | "decoder"` + structured `canonical` sub-object per `contracts/audit-record.md` §1); (b) BLAKE3 digesting of canonical bytes and original bytes (`Blake3Hash` placeholder constructors replaced with real `blake3::hash(...)` values; `blake3` crate added to the workspace dependency graph); (c) closed-set `MessageTemplate` JSON serialization in audit records (`Diagnostic.message: Box<str>` migrated to `Message`; renderer emits `{"template": "...", "args": {...}}` structured shape); (d) `from_parsed_unchecked` adapter deletion (27 call sites migrated to the post-keystone path). **PR 3c.2 is the sole authoritative deletion milestone for `from_parsed_unchecked`; any wording elsewhere in this spec that still refers to deletion at PR 3 is superseded by FR-035a and should be read as PR 3c.2.** The bump remained a clean break per FR-037 (no accept-list, pre-cutover records unreadable). The 2-tuple `RuleId` form was **NOT** part of FR-035a's scope — it landed separately at **T044** (2026-05-22) as a dedicated post-PR-10 PR that unfroze FR-049 for a single atomic change and bumped `marque-1.0 → marque-2.0`; see FR-026 and `docs/refactor-006/legacy-rule-id-map.md`.
...
- **FR-044** (landed at T044, 2026-05-22): Synthetic engine diagnostics carry the sentinel scheme `"engine"` in `(scheme, predicate_id)` form: `RuleId::new("engine", "recognition.decoder-recognized")` for R001, `RuleId::new("engine", "fix.reparse-failed")` for R002. Per T044 PM decision OD-4, the `r001` / `r002` numeric placeholders from the pre-T044 spec wording are **dropped** — the `scheme = "engine"` tuple already carries the cross-version anchor, and descriptive `<class>.<predicate>` reads better at audit-log triage. The `"engine"` scheme is reserved; it is not a valid `MarkingScheme` registration target (alongside the `"test"` scheme reserved for `#[cfg(test)]` fixtures, per T044 PM decisions). Rationale: R001/R002 are minted by the engine, not by a `MarkingScheme` impl — inheriting an active scheme's namespace would lie about provenance. The sentinel keeps `("capco", …)` cleanly meaning "from a CAPCO rule" and is forward-compatible with future schemes (`("cui", …)`, `("nato", …)`, etc.). See `docs/refactor-006/legacy-rule-id-map.md` for the engine-sentinel rename rows.
...
#### PR 3d surface (marking-form vocabulary + validity windows)

Pre-PR-4 follow-on to the keystone window. Lands the per-token form-set and migration-validity-window data plumbing that the longer-term goals — historical/deprecated marking handling, recognize-any-form/emit-correct-form, schema-compliant output — depend on. PR 3d is independent of PR 3.7 (lattice §-resolution spike) and PR 4 (lattice impls): the vocabulary additions are additive and non-conflicting. PR 4 picks up the new `FormSet` shape natively when it lands its own `Vocabulary<S>` extensions (`shape_admits`, `is_fdr_dissem`).

- **FR-053**: `Vocabulary<S>` MUST expose a `forms(&self, token: &S::Token) -> &'static FormSet` accessor. `FormSet` MUST include the three canonical CAPCO §G.1 Table 4 fields: `portion: &'static str` (column 3, always present), `banner_title: &'static str` (column 1, always present — the long Marking Title, e.g., `NOT RELEASABLE TO FOREIGN NATIONALS` for NOFORN, `SECRET` for SECRET), `banner_abbreviation: Option<&'static str>` (column 2, `Some` only when distinct from `banner_title` — e.g., `Some("NOFORN")` for NOFORN, `None` for SECRET). `FormSet` MUST also include `recognized_aliases: &'static [(FormKind, &'static str)]` carrying forms that the engine recognizes on input but does not emit by default (ODNI ISM `Description.title` when it differs from `banner_title`; pre-2018 historical aliases). The existing per-form methods (`portion_form`, `banner_form`, `banner_abbreviation`) MUST remain on the trait as default-method accessors over `forms()` so existing call sites are unaffected — `banner_form()` returns `forms().banner_abbreviation.unwrap_or(forms().banner_title)`, preserving the existing `MarkingForm.banner` "abbreviation when distinct, else title" semantics byte-for-byte. `FormKind` MUST be a closed `#[non_exhaustive]` enum so a recognize-only alias can later be promoted to first-class emission (via a paired `EmissionForm` variant) without an API break — the data shape stays stable as the schema-compliance surface grows.
- **FR-054**: `Deprecation<Token>` MUST carry validity-window fields `valid_from: Option<&'static str>` (schema version of first publication) and `valid_until: Option<&'static str>` (schema version after which the token is deprecated; populated when a successor lands in the migration table). Build-time generation MUST populate both fields from the ODNI XSD annotations and the migration table where information is available. No runtime consumer is required in PR 3d — the data plumbing unblocks the post-refactor "evaluate as valid for the time" mode (a `RuleContext.evaluation_as_of: Option<SchemaVersionId>` follow-on, scoped out of the refactor) without requiring a second migration-generation pass later.
...
- **FR-048**: A bare NATO classification portion appearing in a US-classified document SHOULD carry `REL TO USA, NATO` per CAPCO-2016 §H.7 p127 (#265). Implemented as a hand-written `Suggest`-severity rule (S007 `bare-nato-requires-rel-to-usa-nato`) with portion-level scope; banner roll-up flows automatically from existing portion → banner projection. The solely-NATO-document case is carved out via `PageContext::is_solely_nato_classified`. Authority is example-derived (§H.7 p127 Notional Example 2: `(//CTS//BOHEMIA//REL TO USA, NATO)` — "a NATO COSMIC TOP SECRET (CTS) BOHEMIA portion within a US classified document and is releasable back to NATO"); the previous wording mandated a declarative `Constraint`, which is rejected because `MarkingScheme::evaluate_custom` cannot read `RuleContext::page_context` (see followup `specs/006-engine-rule-refactor/followups/constraint-context-extension.md`).

  **Amendment note (PR 9c.2)**. The original FR-048 wording — "MUST trigger a declarative `Constraint` requiring `REL TO USA, NATO` derivation in the banner" — was wrong on three independent points: (1) the declarative-`Constraint` shape is blocked because `MarkingScheme::evaluate_custom` receives `&CanonicalAttrs` only and cannot read the page context required to enumerate sibling portions for the solely-NATO carve-out; (2) "in the banner" misplaced the scope — the canonical fix is at portion level and banner roll-up flows automatically; (3) "MUST" overstated the citation, which is example-derived (§H.7 p127 Notional Example 2) rather than mandate prose, so `Severity::Suggest` (matching S004/S005/S006 precedent) is the correct floor. This is a non-controversial editorial correction landing with the implementing PR.
...
- **`Diagnostic`**: Surface output. `message: Message { template: MessageTemplate, args: MessageArgs }` (closed enum + closed-set arg types), `severity`, `span`, `citation`, optional `FixIntent<S>`.
...
- **`Vocabulary<S>`**: Per-token metadata surface (existing). Extended by FR-015 (`shape_admits` for open-vocab admission), FR-010 (`is_fdr_dissem` per-token field), and FR-053 (`forms()` accessor returning `FormSet`). Per-form accessors (`portion_form`, `banner_form`, `banner_abbreviation`) become default methods over `forms()` post-3d.
- **`FormSet` / `FormKind`**: Per-token form aggregation. Carries the three CAPCO §G.1 Table 4 columns — `portion` (col 3, always present), `banner_title` (col 1, always present), `banner_abbreviation` (col 2, `Option` — `Some` only when distinct from `banner_title`) — plus a `recognized_aliases` slice for forms recognized on input but not emitted by default (ISM `Description.title`, historical aliases). `FormKind` is `#[non_exhaustive]` so a recognize-only alias can later become a first-class emission target without breaking the data shape.
- **`RenderContext` / `EmissionForm`**: Engine-supplied context to `MarkingScheme::render_canonical`. `EmissionForm` is a `#[non_exhaustive]` closed enum (`Auto | Portion | BannerTitle | BannerAbbreviation` at minimum, named per CAPCO §G.1 Table 4 column terms); `Auto` derives form from `Scope` (Page → `BannerAbbreviation` if present else `BannerTitle`; Portion → `Portion`) to preserve pre-3c.2 behavior. `BannerAbbreviation` falls back to `BannerTitle` when no distinct abbreviation exists. Reserves the future grow-path for `IsmDescriptionTitle`, `XmlAttribute`, and scheme-specific variants.
- **`Deprecation<Token>`**: Per-token deprecation metadata. Extended at PR 3d with `valid_from` / `valid_until` schema-version fields (FR-054). No runtime consumer in 3d; data plumbing unblocks the post-refactor historical-as-valid evaluation mode.
