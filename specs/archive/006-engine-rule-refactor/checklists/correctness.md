...
- [ ] CHK008a - Is the **"US always reciprocates equivalent U.S. protection"** rule (CAPCO §H.7) covered by a named fixture (T063a corpus + T117a property test): `(S//NF) + (//DEU TS//REL TO USA, DEU)` MUST produce banner `TOP SECRET//FGI DEU//NOFORN`. The phrasing is a one-liner for what's otherwise four interacting invariants: (a) level resolves to max across systems via `effective_level()` — the foreign max maps to its US-equivalent (`DEU TS` ↔ `TOP SECRET`); (b) banner system is US (the US-equivalent protection, not a foreign-system banner); (c) FGI marker derives the foreign portion's `MarkingClassification::Fgi.countries`; (d) `noforn-clears-rel-to` PageRewrite collapses the dissem set to `{NOFORN}`. Pre-PR-5 these invariants are individually plausible (level-max test exists at `page_context.rs:1011`) but no end-to-end banner-rendering test pins them together. [Completeness, Spec §FR-007 + FR-008 + FR-009 + FR-012 / CAPCO §H.7] [GATE]
...
- **GATE items**: 23 items tagged `[GATE]` — these MUST clear before the corresponding P1 PR (0.6 citation fix, 3a/3b/3c keystone, 5 / 6 foreign banner, 3.7 lattice spike) merges. Authoritative GATE list (for unambiguous recounts): CHK001, CHK002, CHK006, CHK008a, CHK010, CHK015, CHK018, CHK026, CHK028, CHK032, CHK040, CHK044, CHK045, CHK046, CHK047, CHK051, CHK059, CHK067, CHK068, CHK074, CHK077, CHK078, CHK079. The four Table-3 §D.2 banner-precedence gaps (rules 17, 23, 26, 27 — CHK044/045/046/047) are uniformly gated since `Scope::Page` projection correctness depends on all four. CHK008a (mixed US+foreign portion rollup) is gated because it pins the cross-axis dominance interactions PR 5 / PR 6 build on.
