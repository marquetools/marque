...
- [ ] T001 Add `corpus-override`, `corpus-harness`, `decoder-harness` Cargo features across relevant crates (`crates/capco/Cargo.toml`, `crates/config/Cargo.toml`, `crates/engine/Cargo.toml`, `crates/wasm/Cargo.toml`, `marque/Cargo.toml`), with WASM target explicitly disabling `corpus-override` via `default-features = false`
- [ ] T002 [P] Create mangled-fixture directory tree `tests/fixtures/mangled/{typo,reordering,missing-delimiter,superseded-token,wrong-case,garbled-delimiter}/` with a top-level `tests/fixtures/mangled/README.md` documenting the six classes and fixture schema
- [ ] T003 [P] Create corpus-priors directory `crates/capco/corpus/` with a `README.md` describing the priors JSON format and regeneration command (`python3 tools/corpus-analysis/analyze.py --mode priors --output crates/capco/corpus/priors.json`)
- [ ] T004 [P] Extend `tools/corpus-analysis/analyze.py` (Python tool, not a Cargo crate — the Enron corpus analysis lives in Python and marque consumes its JSON output at build time per foundational-plan §4.4 and §6.1) to expose two new modes: `--mode mangled` emits labeled mangled-marking fixtures to `tests/fixtures/mangled/<class>/`, and `--mode priors` emits the corpus-derived priors table to a caller-specified JSON path. Update `tools/corpus-analysis/README.md` and `requirements.txt` accordingly
- [ ] T004a [P] Extend `crates/capco/build.rs` to read `crates/capco/corpus/priors.json` via `serde_json` at compile time and emit `&'static` corpus-priors tables into `OUT_DIR/priors.rs` (included via `crates/capco/src/priors.rs`). Build fails closed if the JSON is missing or malformed — matches foundational-plan line 679 ("build.rs emits the baked frequency table"). No runtime JSON parsing; no runtime `serde_json` runtime dep
- [ ] T005 [P] Add compile-time audit-schema selection via build script env var in `crates/engine/build.rs` (`MARQUE_AUDIT_SCHEMA`, defaulting to `marque-mvp-2`), emitting `cargo:rustc-env=MARQUE_AUDIT_SCHEMA=...`
...
- [ ] T042 [US2] Run `python3 tools/corpus-analysis/analyze.py --mode priors --output crates/capco/corpus/priors.json` to produce `crates/capco/corpus/priors.json`; commit this artifact (depends on T040)
...
- [ ] T060 [US2] Bake corpus priors from `crates/capco/corpus/priors.toml` into `&'static` Rust tables at build time in `crates/capco/build.rs`, emitting tables into `crates/capco/src/priors.rs` via `include!()`
