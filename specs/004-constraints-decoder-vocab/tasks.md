...
- [ ] T055 [P] [US2] Audit v2 single-version invariant test `single_schema_per_build` in `crates/engine/tests/audit.rs` (no v1 records appear in v2-build output stream). **Build-layer half landed**: `crates/engine/build.rs` validates `MARQUE_AUDIT_SCHEMA` to `["marque-mvp-1", "marque-mvp-2"]` and panics on unknown values, so a single build cannot mix versions by construction. **Runtime-emitter half blocked on PR-4**: `marque/src/render.rs` and `crates/wasm/src/lib.rs` still hardcode `AUDIT_SCHEMA_VERSION = "marque-mvp-1"` (whitepaper §980 P0-1) so the output stream is v1 regardless of the env var. The stream-level invariant test lands with PR-4 alongside the emitter wiring.
