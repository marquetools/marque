...
- [ ] T077 [P] [US3] Zero-allocation metadata query test `metadata_query_is_zero_alloc` in `crates/capco/tests/vocabulary_zero_alloc.rs` (uses allocation-counter instrumentation; asserts repeated `metadata()` calls allocate 0 bytes). **Implementation note (PR-2):** lives in its own integration file gated at the FILE level on `#![cfg(feature = "count-allocs")]` so the counting global allocator is not shared with other tests. Mirrors the discipline in `crates/core/tests/alloc_budget.rs` (gap register #15).
- [ ] T077a [P] [US3] **Expanded `Vocabulary<CapcoScheme>` coverage** in `crates/capco/tests/vocabulary.rs` covering the production code paths the original T071–T077 set leaves uncovered: (a) `banner_abbreviation_some_for_distinct_form` over `NF`/`UCNI`/`ND`/`XD` (CAPCO-2016 §G.1 Table 4 distinct-abbreviation rows); (b) `banner_abbreviation_none_for_same_form` over `RD`/`FRD`/`TFNI`/`RD-CNWDI`/`HCS`/`R` (no distinct authorized abbreviation); (c) `metadata_agrees_with_per_field_accessors` (single-source-of-truth invariant — `metadata(t).{authority, owner_producer, point_of_contact, portion_form, banner_form, banner_abbreviation}` must match the per-field accessors); (d) seven `#[should_panic]` tests covering every accessor's unknown-TokenId chokepoint (`authority`, `owner_producer`, `point_of_contact`, `metadata`, `portion_form`, `banner_form`, `deprecation`) using aggregate / trigraph / grammar-shape sentinels (`TOK_FGI_MARKER`, `TOK_USA`, `TOK_JOINT`, `TOK_US_CLASSIFIED`, `TOK_NON_US_CLASSIFICATION`, `TOK_IC_DISSEM`, `TOK_NON_IC_DISSEM`) that are deliberately absent from `SENTINEL_TO_CANONICAL`. Closes the patch-coverage gap codecov flagged on PR #143 (~37 lines uncovered prior to this task).
