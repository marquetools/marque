...
- [X] T027 [US1] Implement `evaluate<S: MarkingScheme>(constraints, marking) -> Vec<ConstraintViolation>` free function in `crates/scheme/src/constraint.rs` (deterministic, declaration-ordered, no per-constraint allocation beyond output vec). **Implementation note:** the signature is `evaluate<S>(scheme: &S, marking: &S::Marking)` — the evaluator walks `scheme.constraints()` and dispatches `TokenRef` resolution through a new `MarkingScheme::satisfies(marking, token_ref)` trait method, and `Constraint::Custom` through `MarkingScheme::evaluate_custom(name, marking)`. Original spec's two-arg `evaluate(constraints, marking)` signature was infeasible because token-presence resolution needs scheme-specific knowledge.
...
- [ ] T035 [US1] **DEFERRED from Phase 3**: Retire the 12 corresponding hand-written `Rule` impls from `crates/capco/src/rules.rs` (`BareHcsRule`, `DualClassificationRule`, `NonUsMissingDissemRule`, `JointRestrictedRule`, `JointFgiRule`, `JointIcDissemRule`, `JointNonIcDissemRule`, `AeaNofornRule`, `CnwdiConstraintRule`, `RdPrecedenceRule`, `UcniClassificationRule`, `CominglingWarningRule`) and remove their registrations from `CapcoRuleSet::new()`. Retire E014 `JointRelToRule` and the lattice-side NOFORN⊐REL-TO / FGI-absorption code. Net: 15 retirements (12 + E014 + 2 lattice behaviors → 3 `PageRewrite` entries; rule count drops 39 → 24). **Deferral rationale:** each retired rule emits per-rule formatted diagnostic messages with rule-specific span selection; constructing a generic constraint-evaluator pipeline that preserves byte-identical output requires carrying rule_id/severity/message-template/span-selector metadata across the scheme→engine boundary, which is a substantially larger surface change than Phase 3's declarative-data landing. The declarative `Constraint` + `PageRewrite` entries added by T033/T034 establish the catalog; T035 lands as a separate follow-up PR once the ConstraintViolation → Diagnostic conversion layer is designed.
