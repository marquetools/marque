...
- [X] T048 [P] [US2] **Complete (PR-3).** Interactive-no-decoder test landed as `crates/engine/tests/decoder_dispatch.rs::lint_without_deep_scan_never_invokes_the_decoder`. `Engine::new(...)` without `with_deep_scan()` hardcodes `strict_evidence: true` in `ParseContext`, which `StrictOrDecoderRecognizer` treats as "skip decoder fallback" even when the dispatcher is installed — so `FixSource::DecoderPosterior` can never appear. A canonical-input regression (`deep_scan_does_not_change_canonical_input_diagnostics`) pins that `with_deep_scan()` is a no-op when the strict path already resolves.
