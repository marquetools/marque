...
- [X] T053 [P] [US2] Audit v2 decoder-path record test `decoder_path_record_shape` in `crates/engine/tests/audit.rs` (asserts `confidence.recognition < 1.0_f32`, non-empty `confidence.features` with enum-typed `FeatureId`, `source == DecoderPosterior`, `confidence.runner_up_ratio == Some(r)` with finite `r`)
