...
- [x] T053 [US1] Reserved slots baked into `marque-mvp-3` schema (per PR 3c.B Commit 10) for `FeatureId::PrecedingFixPenalty` (PR 7c fills) and `MessageTemplate::ReparseFailed` (R002, PR 7b filled). The full `marque-1.0` shape with the four `marque-1.0`-only commitments (Canonical wire-up + BLAKE3 + closed MessageTemplate JSON + `from_parsed_unchecked` delete) lands at PR 3c.2 per FR-035a. (PR-3c.B Commit 10 landed; PR-3c.2 pending)
