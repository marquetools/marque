...
> PR 3b proper targets **13–18 surviving rules** (Stage 1 of the
> staging table); end-state 8–18 acceptance lands post-Stage-4.

- [ ] T026 [US1] **(re-sequenced 2026-05-07)** PR 3b umbrella task — see T026a–T026f for the six sub-moves. Source plan #263 supersedes the "~56 → ~10–13" wording with the `marque-applied.md` §3 / §3.11 staging. Each sub-move is independently committable inside PR 3b; partial PR 3b landings (e.g., T026a but not T026f) are permitted if the merged subset passes T029's CI matrix and the running rule count stays at or below the upper bound (PR-3b)
- [ ] T026a [US1] **PR 3b Sub-move A — banner-roll-up walker.** Collapse E031 / E034 / E035 / E040 / E045 (banner-roll-up rules across SCI / SAR / dissem / FGI / classification) to ONE generic `BannerMatchesProjectedRule` calling `MarkingScheme::project(Scope::Page, &portions) -> ProjectedMarking` and comparing to the observed banner; emit a single diagnostic per category-class-of-mismatch. The walker retires at PR 4 when per-category Lattice impls + property tests in `proptest_lattice.rs` land. **Single CAPCO-§ rule citation per category catalog row** (D13 discipline) — the operative banner-roll-up rule per category: §D.2 for classification (the "banner = max of portions" rule; §A.4 defines the levels but is not the rule citation), §A.6 for SCI banner roll-up, §H.4 for SCI per-system, §H.5 for SAR, §H.7 for FGI, §H.8 for dissem, per `marque-applied.md` §3.1. Background §-references (where a token is defined, related-fact discussion) MAY appear in row documentation but are NOT counted toward the D13 single-citation discipline; only the operative banner-roll-up §-citation is the row's "primary." Reviewer audible: skip 3b.A if PR 4 reviewer accepts property-test-only coverage of the banner-equality invariant; document the choice in PR description. (FR-006; PR-3b)
...
- [ ] T026c [US1] **PR 3b Sub-move C — RELIDO `Constraint::Conflicts` enumerated form.** Land `marque-applied.md` §3.4.2 RELIDO incompatibility roster as ~15–20 enumerated `Constraint::Conflicts` rows with single-token RHS (existing `Constraint` variant; no primitive change). Two groupings per `marque-applied.md` §3.4.2: FD&R-family domination (NOFORN / LES-NF / SBU-NF / DISPLAY ONLY) + non-US-family conflict (one row per FGI / JOINT / NATO atom). **Single CAPCO-§ rule citation per row** (D13 discipline): §H.8 p154 — the FD&R supersession chain rule that establishes RELIDO as the bottom of the FD&R axis and therefore incompatible with FD&R-dominators and non-US-equity tokens. This is the operative rule for every row in the roster. Background §-references documenting where each RHS token is defined (e.g., §H.9 p185 for LES-NF, §H.8 p146 for DISPLAY ONLY, §H.7 for FGI atoms, §H.3 p56 for JOINT atoms, §H.7 Appendix B for NATO atoms) MAY appear in row documentation as background but are NOT counted toward the D13 single-citation discipline. The rows compact to 2 family-predicate rows in PR 4 once T108b's `RhsFamily(predicate)` variant ships. (FR-018; PR-3b)
- [ ] T026d [US1] **PR 3b Sub-move D — class-floor `Constraint::Requires` catalog.** Land `marque-applied.md` §3.4.6 per-token classification floor catalog as ~25 `Constraint::Requires` rows on `CapcoScheme` (hand-curated; per-row CAPCO §H.x pNN citation). Closure-implied requirements (the implicit-default trio + per-marking unconditional implications from `marque-applied.md` §4.7) stay as `Requires` rows in PR 3b; the closure operator primitive (T108c) re-classifies the implication-shaped entries from `Requires` to closure entries in PR 4. Q-3.4.6a (build-time generation from CVE/Schematron metadata) deferred to a follow-up spike — not blocking PR 3b. (FR-018; PR-3b)
- [ ] T026e [US1] **PR 3b Sub-move E — SCI per-system collapse.** Collapse the 10 rules in `crates/capco/src/rules_sci_per_system.rs` (E042–E051) to ONE `Constraint::Custom("sci-per-system", ...)` walker dispatching over a per-system catalog (data-only). Each catalog row covers one SCI control system with its §H.4 row Y citation; walker body is one branch per per-system constraint kind. Aligns with `marque-applied.md` §3.10.3 Move 4 / Aggressive consolidation step 2. (FR-018; PR-3b)
- [ ] T026f [US1] **PR 3b Sub-move F (optional retention) — non-canonical input walker.** Per Q-Move-7-timing default: retain ONE "non-canonical input" diagnostic walker covering E020 / E023 / E028 / E033 ordering checks (REL TO leads with USA, AEA SIGMA numeric sort, SAR alphabetic, SCI numeric-then-alpha) as a single `impl Rule` block until the renderer trait surface lands in PR 5+. Walker body is one branch per ordering-check kind (≤3 per D13). Single citation per catalog row (§A.6 / §H.5 / §H.6 / §H.8). Walker retires when PR 5+ renderer takes over (Stage 4). (FR-018; PR-3b)
- [ ] T027 [US1] Update rule registration in `CapcoRuleSet::new()` to reflect the collapsed catalog produced by T026a–T026f; preserve every distinct citation (FR-018) and severity. Reviewer attests in PR description: (a) **single CAPCO-§ citation per declarative catalog entry** (Q-3.9 default — `plan.md` D13 addendum), (b) **predicate body of every `impl Rule` block has ≤3 internal branches**, (c) **post-3b rule count in 13–18** (Stage 1 target). Out-of-band counts require team review per `plan.md` D13 (PR-3b)
- [ ] T028 [P] [US1] Update test fixtures expecting rules removed by collapse; consolidate per-rule unit-test modules where rules merged. Per-sub-move test reshape: T026a tests collapse to a single banner-projection-equivalence test bundle; T026b tests reshape to per-`PageRewrite` declaration tests; T026c–T026e tests reshape to per-catalog-entry test fixtures asserting the walker fires once per matching row (PR-3b)
