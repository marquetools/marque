...
> the five sub-moves T026a–T026e named in `plan.md` D13 addendum.
...
- [ ] T026 [US1] **(re-sequenced 2026-05-07)** PR 3b umbrella task — see T026a–T026e for the five sub-moves. Source plan #263 supersedes the "~56 → ~10–13" wording with the `marque-applied.md` §3 / §3.11 staging. Each sub-move is independently committable inside PR 3b; partial PR 3b landings (e.g., T026a but not T026e) are permitted if the merged subset passes T029's CI matrix and the running rule count stays at or below the upper bound (PR-3b)
- [ ] T026a [US1] **PR 3b Sub-move A — banner-roll-up walker.** Collapse E031 / E034 / E035 / E040 / E045 (banner-roll-up rules across SCI / SAR / dissem / FGI / classification) to ONE generic `BannerMatchesProjectedRule` calling `MarkingScheme::project(Scope::Page, &portions) -> ProjectedMarking` and comparing to the observed banner; emit a single diagnostic per category-class-of-mismatch. The walker retires at PR 4 when per-category Lattice impls + property tests in `proptest_lattice.rs` land. Single citation per category catalog row (each row cites one of §A.6 / §H.4 / §H.5 / §H.7 / §H.8 / §A.4 + §D.2 per `marque-applied.md` §3.1). Reviewer audible: skip 3b.A if PR 4 reviewer accepts property-test-only coverage of the banner-equality invariant; document the choice in PR description. (FR-006; PR-3b)
