...
- [ ] T048a [US1] Define `RenderContext { scope, emission_form, schema_version }` and `#[non_exhaustive] enum EmissionForm { Auto, Portion, BannerTitle, BannerAbbreviation }` in `crates/scheme/src/scheme.rs` alongside the `MarkingScheme::render_canonical` declaration. Variant naming follows CAPCO §G.1 Table 4 column terms (col 1 "Authorized Banner Line Marking Title" → `BannerTitle`; col 2 "Authorized Banner Line Abbreviation" → `BannerAbbreviation`; col 3 "Authorized Portion Mark" → `Portion`). `EmissionForm::Auto` is the documented default that preserves pre-3c.2 emission behavior (Page → `BannerAbbreviation` if present else `BannerTitle`; Portion → Portion). `#[non_exhaustive]` reserves the future grow-path for `IsmDescriptionTitle` (post-PR-3d, when ISM `Description.title` divergence needs to be emittable for schema compliance), `XmlAttribute` (post-refactor ISM-XML codec), and scheme-specific variants without an API break. `schema_version: SchemaVersionId` is reserved data plumbing — engine wires it from the active scheme's pinned `ism-schema-version`; no consumer in 3c.2. (PR-3c.2, FR-052)
...
- [x] T052 [US1] **AMENDED 2026-05-14 per D-7.18**: PR 3c.B Commit 10 actually cut `MARQUE_AUDIT_SCHEMA` from `marque-mvp-2` to **`marque-mvp-3`** (conservative bump). The `marque-mvp-3 → marque-1.0` cutover defers to **PR 3c.2** per amended FR-035 + new FR-035a. Single-value validation, no accept-list, preserved at both stages. (FR-034, FR-035, FR-035a; PR-3c.B Commit 10 landed; PR-3c.2 pending)
- [x] T053 [US1] Reserved slot baked into `marque-mvp-3` schema (per PR 3c.B Commit 10) for `MessageTemplate::ReparseFailed` (R002, PR 7b filled). The originally-reserved `FeatureId::PrecedingFixPenalty` slot was retired during PR 7c (D-7.22; misunderstanding-derived per PM clarification 2026-05-14). The full `marque-1.0` shape with the four `marque-1.0`-only commitments (Canonical wire-up + BLAKE3 + closed MessageTemplate JSON + `from_parsed_unchecked` delete) lands at PR 3c.2 per FR-035a. (PR-3c.B Commit 10 landed; PR-3c.2 pending)
- [ ] T054 [US1] Delete `from_parsed_unchecked` adapter; rules consume `&CanonicalAttrs` constructed only via the explicit `MarkingScheme::canonicalize(ParsedAttrs<'_>) -> CanonicalAttrs` path. **Deferred from PR 3c → PR 3c.2 per amended FR-035 + new FR-035a** (2026-05-14 audit confirms 27 surviving call sites). (PR-3c.2)
...
**Checkpoint** (amended 2026-05-14): Audit-record content-ignorance is a type invariant (achieved at PR 3c.B Commit 10's `marque-mvp-3` bump); canary scan over five-corpus sweep returns zero leaks; legacy rule-IDs retired. **The `marque-mvp-3 → marque-1.0` cutover deferred to PR 3c.2** (Canonical wire-up + BLAKE3 + closed MessageTemplate JSON + `from_parsed_unchecked` delete). The 2-tuple `(scheme, predicate-id)` rule-ID form deferred further still to post-PR-10 per FR-049.
