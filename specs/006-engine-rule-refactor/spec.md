...
- **FR-052**: `MarkingScheme::render_canonical`'s `RenderContext` parameter MUST carry an explicit `EmissionForm` selector — a closed `#[non_exhaustive]` enum with at minimum `Auto | Portion | Banner | BannerAbbreviated | LongTitle`. `EmissionForm::Auto` MUST derive the emission form from `Scope` (Page → Banner; Portion → Portion), preserving pre-3c.2 emission behavior for every existing `FixIntent` site. The closed-CVE branch of `render_canonical` MUST honor the selector by routing to `Vocabulary<S>::portion_form` / `banner_form` / `banner_abbreviation` / (PR 3d) the long-title field on `FormSet`. `#[non_exhaustive]` reserves the budget for future variants (e.g., `IsmDescriptionTitle`, `XmlAttribute`, scheme-specific forms) without an API break — adding a variant MUST NOT require touching `FixIntent` emission sites that pass `EmissionForm::Auto`. Rationale: `render_canonical` is the single open-vocab emission path being introduced in PR 3c.2 (T048); locking the selector shape now is essentially free, retrofitting later means re-signing `render_canonical` and propagating into every rule's `FixIntent` emission.
...
- **FR-053**: `Vocabulary<S>` MUST expose a `forms(&self, token: &S::Token) -> &'static FormSet` accessor. `FormSet` MUST include the canonical portion form, banner form, optional banner abbreviation, optional CAPCO long Marking Title, and a `recognized_aliases: &'static [(FormKind, &'static str)]` slice carrying forms that the engine recognizes on input but does not emit by default (e.g., ODNI ISM `Description.title` when it differs from the CAPCO long-title; pre-2018 historical aliases). The existing per-form methods (`portion_form`, `banner_form`, `banner_abbreviation`) MUST remain on the trait as default-method accessors over `forms()` so existing call sites are unaffected. `FormKind` MUST be a closed `#[non_exhaustive]` enum so a recognize-only alias can later be promoted to first-class emission (via a paired `EmissionForm` variant) without an API break — the data shape stays stable as the schema-compliance surface grows.
...
- **`FormSet` / `FormKind`**: Per-token form aggregation. Carries portion / banner / optional banner-abbreviation / optional long-title forms plus a `recognized_aliases` slice for forms recognized on input but not emitted by default (ISM `Description.title`, historical aliases). `FormKind` is `#[non_exhaustive]` so a recognize-only alias can later become a first-class emission target without breaking the data shape.
- **`RenderContext` / `EmissionForm`**: Engine-supplied context to `MarkingScheme::render_canonical`. `EmissionForm` is a `#[non_exhaustive]` closed enum (`Auto | Portion | Banner | BannerAbbreviated | LongTitle` at minimum); `Auto` derives form from `Scope` to preserve pre-3c.2 behavior. Reserves the future grow-path for `IsmDescriptionTitle`, `XmlAttribute`, and scheme-specific variants.
