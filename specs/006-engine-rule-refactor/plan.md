...
**Storage**: N/A on the hot path. Build-time cache via Cargo `OUT_DIR`. ~~The planned LMDB `LintResult` cache is out of scope for this refactor.~~ **[Superseded: the LMDB `LintResult` cache was descoped — constitution v1.8.0; Marque persists no derived result to disk.]**
