...
**Vulnerability:** The `crates/ism/src/attrs/country_code.rs` file contained an `unsafe` block using `std::str::from_utf8_unchecked` when a safe alternative `std::str::from_utf8(...).unwrap()` was available, and the `crates/ism/src/lib.rs` used `#![deny(unsafe_code)]` instead of the stricter `#![forbid(unsafe_code)]`.
...
**Prevention:** Always use safe Rust equivalents (like `.unwrap()` on UTF-8 conversion when bytes are known to be ASCII) unless there is a proven, measured performance bottleneck that absolutely requires `unsafe`. Enforce this codebase-wide by using `#![forbid(unsafe_code)]` instead of `#![deny(unsafe_code)]`.
