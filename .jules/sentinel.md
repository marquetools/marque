...
## 2026-05-16 - [HIGH] Command Injection in Demo Recorder
**Vulnerability:** `demo/record-demo.js` invoked `execSync` with a shell-interpolated template string containing the unsanitized `outFile` variable. An attacker controlling the output path could inject shell metacharacters to execute arbitrary commands.
**Learning:** Passing user-controlled or externally-derived values into a shell command string via template literals is a classic injection vector. The shell tokenizes the entire string, so any metacharacter (`;`, `|`, `$()`, etc.) embedded in a variable value executes as a new command or subshell.
**Prevention:** Use `execFileSync` (or `spawn`) with the command name as the first argument and an explicit `string[]` of arguments as the second. This bypasses the shell entirely — arguments are passed directly to the OS `execve` syscall with no tokenization or interpolation.
## 2026-06-15 - [MEDIUM] Missing Strict Security Headers on REST API
**Vulnerability:** The `marque-server` axum REST API lacked the `Content-Security-Policy` and `Strict-Transport-Security` headers.
**Learning:** While the API is primarily designed for automated clients, adding a highly restrictive CSP (`default-src 'none'; frame-ancestors 'none';`) and HSTS header (`max-age=63072000; includeSubDomains`) hardens the API against MIME-sniffing, cross-site scripting (if mistakenly accessed by a browser or if a vulnerability in content types arises), and downgrade attacks. Defense-in-depth principles dictate that API endpoints should be as tightly constrained as possible.
**Prevention:** Use `tower_http::set_header::SetResponseHeaderLayer` to globally enforce `Content-Security-Policy` and `Strict-Transport-Security` across all axum routes in the router configuration. Ensure `tower-http` has the `set-header` feature enabled.
