...
**Learning:** While the API is primarily designed for automated clients, adding a highly restrictive CSP (`default-src 'none'; frame-ancestors 'none';`) reduces browser-side script/embedding risk if endpoints are ever accessed in a browser context, and HSTS (`max-age=63072000; includeSubDomains`) specifically mitigates HTTPS downgrade attacks. Defense-in-depth principles dictate that API endpoints should be as tightly constrained as possible.
