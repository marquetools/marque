...
## 2026-05-23 - [HIGH] Removing Unnecessary Unsafe Code
**Vulnerability:** The `crates/ism/src/attrs/country_code.rs` file contained an `unsafe` block using `std::str::from_utf8_unchecked` when a safe alternative `std::str::from_utf8(...).expect("…")` was available, and the `crates/ism/src/lib.rs` used `#![deny(unsafe_code)]` instead of the stricter `#![forbid(unsafe_code)]`.
**Learning:** Using `unsafe` when safe alternatives exist introduces unnecessary risk of Undefined Behavior if invariants (like assuming bytes are valid ASCII/UTF-8) are ever broken. Furthermore, `#![deny(unsafe_code)]` can be overridden locally with `#[allow(unsafe_code)]`, whereas `#![forbid(unsafe_code)]` cannot, providing a stronger security guarantee.
**Prevention:** Always use safe Rust equivalents (like `std::str::from_utf8(...).expect("invariant violated")` when bytes are known to be ASCII) unless there is a proven, measured performance bottleneck that absolutely requires `unsafe`. Enforce this codebase-wide by using `#![forbid(unsafe_code)]` instead of `#![deny(unsafe_code)]`.
