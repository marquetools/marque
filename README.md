...
| [`marque-capco`](./crates/capco/) | 50 CAPCO rules (errors `E001`–`E059` with `E017`/`E018`/`E019` retired in T035b, `E022`/`E025`/`E027` retired in PR 3b.D into the `E058` class-floor catalog walker, and `E042`–`E051` retired in PR 3b.E into the `E059` SCI per-system catalog walker; style `S001`–`S006`; warnings `W002`–`W003` with `W001` retired in T035c-14; corrections `C001`) consuming `marque-ism` predicates. Includes SAR (Special Access Required) validation per §H.5, structural SCI compartment + sub-compartment support per §A.6, per-SCI-system §H.4 companion-required and forbid-companion constraints enforced via the `E059` declarative walker over a 5-row catalog (HCS-O / HCS-P / HCS-P [SUB] / SI-G / TK-{BLFH\|IDIT\|KAND}), NODIS/EXDIS mutual-exclusion + NOFORN-required constraints per §H.9, REL TO list-grammar invariants per §H.8 (ordering E020 + no-duplicates E052), and the suggest-don't-fix channel (S004 `rel-to-trigraph-suggest`, issue #235). |
...
**MVP complete**. Full lint → fix → audit pipeline for raw text with 50 CAPCO
