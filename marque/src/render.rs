...
use marque_engine::{AUDIT_SCHEMA_IS_V3_2, AUDIT_SCHEMA_VERSION, LintResult};
...
//
// The audit-record projection itself is owned by the engine
...
pub fn audit_line_to_json_v1_0(
    ...
    // Single source of truth: the engine owns the audit-record
    // projection (`crates/engine/src/audit_render.rs`). The CLI keeps
    // this thin wrapper so the stderr-I/O + error-frame machinery in
    // `render_audit_line` / `main.rs` does not have to learn the engine
    // module path, but the wire bytes come from the one canonical
    // projection — there is no CLI-private copy to drift.
    marque_engine::audit_line_to_json_v1_0(scheme, line)
}
