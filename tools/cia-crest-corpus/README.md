...
`tests/corpus/capco/documents/`. The pipeline pulls declassified CIA CREST PDFs from
...
make_specs.py      → tests/corpus/capco/documents/specs/<stem>.md
...
render_corpus.py   → tests/corpus/capco/documents/marked/<stem>.md
                     tests/corpus/capco/documents/ground_truth.json (aggregate)
                     tests/corpus/capco/documents/<stem>.expected.json (per doc)
```
...
under `tests/corpus/capco/documents/` is what gets committed.
